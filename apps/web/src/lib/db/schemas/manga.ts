...
export const mangaTable = pgTable(
  ...
  (table) => [
    ...
    index("mangaStatusIndex").on(table.status),
    index("mangaTypeIndex").on(table.type),
    index("mangaGenresIndex").on(table.genres),
    index("mangaChaptersIndex").on(table.chapters),
    index("mangaVolumesIndex").on(table.volumes),
    index("mangaScoreByIndex").on(table.score),
    index("mangaScoredByIndex").on(table.scored_by),
    index("mangaRankIndex").on(table.rank),
    index("mangaPopularityIndex").on(table.popularity),
    index("mangaFavoritesIndex").on(table.favorites),
  ],
);
