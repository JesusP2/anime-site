...
export function SongAutocomplete({ songs, onSelectedValueChange }: {
  songs: {
    id: string;
    title: string;
    animeTitle: string;
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      ...
      if (cache.current[debouncedSearch]) {
        const filtered = cache.current[debouncedSearch].filter(d => !songs.some(s => s.id === d.key))
        setItems(filtered)
        setIsLoading(false)
        return;
      }
      ...
      const data = await res.json() as { id: string; animeTitle: string; link: string; songName: string; }[];
